...
try {
    ...
    $response['rdv_aujourdhui'] = $stmt->fetch()['total'] ?? 0;
    ...
    // Agenda semaine
    $stmt = $pdo->prepare("
        SELECT c.date, DAYOFWEEK(c.date) as jour, TIME(c.heure_debut) as heure_debut, CONCAT(u.prenom, ' ', u.nom) as patient
        FROM creneau c
        JOIN reservation r ON c.id = r.id_creneau
    ...
    foreach ($rdv as $r) {
        $jour = $r['jour'];
        if (!isset($agenda[$jour])) {
            $agenda[$jour] = ['jour_semaine' => $jour, 'evenements' => []];
        }
    ...
    // Patients récents
    $stmt = $pdo->prepare("
        SELECT DISTINCT u.id, u.nom, u.prenom, COUNT(r.id) as nb_consultations, MAX(c.date) as derniere_visite
        FROM reservation r
        JOIN creneau c ON r.id_creneau = c.id
        JOIN utilisateur u ON r.id_etudiant = u.id
        WHERE c.id_praticien = ?
    ...
    $stmt = $pdo->prepare("
        SELECT r.id, TIME(c.heure_debut) as heure_debut, CONCAT(u.prenom, ' ', u.nom) as patient, s.nom as service, r.statut
        FROM creneau c
        JOIN reservation r ON c.id = r.id_creneau
        JOIN utilisateur u ON r.id_etudiant = u.id
    ...
    foreach ($reservations as &$r) {
        $r['heure'] = substr($r['heure_debut'], 0, 5);
    }
    $response['reservations_jour'] = $reservations;
    
    // Disponibilités à venir
    $stmt = $pdo->prepare("
        SELECT id, date, heure_debut, heure_fin FROM creneau
